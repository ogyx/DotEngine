...
namespace DotEngine
{
	...
	{
		DOT_CORE_ERROR("GLFW Error ({0}: {1}", error, descr);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		DOT_CORE_INFO("Create window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized)
		{
			// TODO: glfwTerminate on system shutdown
			int success = glfwInit();
			DOT_CORE_ASSERT(success, "Could not initialize GLFW!");
